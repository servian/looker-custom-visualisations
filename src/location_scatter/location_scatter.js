...
looker.plugins.visualizations.add({
    create: function (element, config) {
        ...
        this.updateScatter = function (svgCanvas, inputForPlayerMap) {
            // var hexBins = inputForPlayerMap.map(d => (d.teamName = d3.mean(d, d => d.teamName == 'St. George Illawarra Dragons' ? 1 : -1), d));
...
updateAsync: function (data, element, config, queryResponse, details, done) {
    ...
    // _chartSvg.selectAll("#fieldMarkings")
    //     .data([fieldJson])
    //     .join("path")
    //         .attr("d", fieldPath)
    //         .attr("fill", "#8ed7b3")
